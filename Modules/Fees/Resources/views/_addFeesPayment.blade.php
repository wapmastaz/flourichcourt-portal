              ...
              @if (isset(Auth::user()->id))
                <div class="single-meta">
                  <div class="d-flex justify-content-between">
                    <div class="name">@lang('wallet::wallet.wallet_balance')</div>
                    <div class="value">
                      {{ generalSetting()->currency_symbol }}{{ Auth::user()->wallet_balance != null ? number_format(Auth::user()->wallet_balance, 2, '.', '') : 0.0 }}
                    </div>
                  </div>
                </div>
                <input type="hidden" name="wallet_balance"
                  ...
                  @if (isset($invoiceInfo))
                    ...
                    @foreach ($invoiceDetails as $key => $invoiceDetail)
                      <tr>
                        ...
                        <td>
                          @if ($invoiceDetail->feesType->is_mandatory)
                            <button class="primary-btn small bg-success text-white border-0">Yes</button>
                          @else
                            <button class="primary-btn small bg-danger text-white border-0">No</button>
                          @endif
                        </td>
                        <td>
                          <div class="input-effect">
                            <input
                              class="primary-input border-0 form-control addFeesAmount{{ $errors->has('amount') ? ' is-invalid' : '' }}"
                              type="text" name="amount[]" autocomplete="off"
                              value="{{ isset($invoiceDetail) ? $invoiceDetail->amount : old('amount') }}" readonly>
                        ...
                        <td>
                          <div class="input-effect">
                            <input class="primary-input border-1 form-control showTotalValue" type="text"
                              name="due[]" value="{{ isset($invoiceDetail) ? $invoiceDetail->due_amount : '' }}"
                        ...
                        <td>
                          @if (isset($role) && $role == 'admin')
                            <div class="input-effect">
                              <input class="primary-input border-sm form-control addFeesWeaver" type="text"
                                name="weaver[]" autocomplete="off"
                          ...
                          @else
                            <input class="primary-input border-0 form-control"
                              value="{{ isset($invoiceDetail) ? $invoiceDetail->weaver : 0 }}" autocomplete="off"
                              readonly>
                          @endif
                        </td>
                        <td>
                          @if (isset($role) && $role == 'admin')
                            <input class="primary-input form-control addFeesFine" type="text" name="fine[]"
                              autocomplete="off" value="0">
                          @else
                            <input class="primary-input border-0 form-control"
                              value="{{ isset($invoiceDetail) ? $invoiceDetail->fine : 0 }}" autocomplete="off"
                              readonly>
                          @endif
                        </td>
                        <td>
                          <button class="primary-btn icon-only fix-gr-bg" data-toggle="modal" data-tooltip="tooltip"
                            data-target="#addNotesModal{{ $invoiceDetail->fees_type }}" type="button"
                          ...
                          <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                                <div class="input-effect">
                                  <input class="primary-input form-control has-content" type="text" name="note[]"
                                    autocomplete="off">
            ...
            <div class="col-lg-12">
              <select class="niceSelect w-100 bb form-control{{ $errors->has('bank') ? ' is-invalid' : '' }}"
                ...
                @foreach ($bankAccounts as $bankAccount)
                  <option value="{{ $bankAccount->id }}" {{ old('bank') == $bankAccount->id ? 'selected' : '' }}>
          ...
          <div class="stripPayment d-none">
              ...
              <div class="col-lg-12">
                <div class="input-effect">
                  <input class="primary-input form-control{{ $errors->has('name_on_card') ? ' is-invalid' : '' }}"
                    type="text" name="name_on_card" autocomplete="off" value="{{ old('name_on_card') }}">
                  <label>@lang('accounts.name_on_card') <span>*</span> </label>
                  <span class="focus-border"></span>
                  @if ($errors->has('name_on_card'))
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $errors->first('name_on_card') }}</strong></span>
                  @endif
                </div>
              </div>
            </div>
            ...
            <div class="row mt-25">
              <div class="col-lg-12">
                <div class="input-effect">
                  <input
                    class="primary-input form-control{{ $errors->has('card-number') ? ' is-invalid' : '' }} card-number"
                    type="text" name="card-number" autocomplete="off" value="{{ old('card-number') }}">
                  <label>@lang('accounts.card_number') <span>*</span> </label>
                  <span class="focus-border"></span>
                  @if ($errors->has('card-number'))
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $errors->first('card-number') }}</strong></span>
                  @endif
                </div>
              </div>
            </div>
              ...
              <div class="col-lg-12">
                <div class="input-effect">
                  ...
                  @if ($errors->has('card-cvc'))
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $errors->first('card-cvc') }}</strong></span>
                  @endif
                </div>
              </div>
            </div>
            ...
            <div class="row mt-25">
              <div class="col-lg-12">
                <div class="input-effect">
                  <input class="primary-input form-control card-expiry-month" type="text" name="card-expiry-month"
                    autocomplete="off" value="{{ old('card-expiry-month') }}">
                  <label>@lang('accounts.expiration_month') <span>*</span> </label>
                  <span class="focus-border"></span>
                  @if ($errors->has('card-expiry-month'))
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $errors->first('card-expiry-month') }}</strong></span>
                  @endif
                </div>
              </div>
            </div>
            ...
            <div class="row mt-25">
              <div class="col-lg-12">
                <div class="input-effect">
                  <input class="primary-input form-control card-expiry-year" type="text" name="card-expiry-year"
                    autocomplete="off" value="{{ old('card-expiry-year') }}">
                  <label>@lang('accounts.expiration_year') <span>*</span> </label>
                  <span class="focus-border"></span>
                  @if ($errors->has('card-expiry-year'))
                    <span class="invalid-feedback" role="alert">
                      <strong>{{ $errors->first('card-expiry-year') }}</strong></span>
                  @endif
                </div>
              </div>
            </div>
