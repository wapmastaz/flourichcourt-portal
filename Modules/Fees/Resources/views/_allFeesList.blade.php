        ...
        <table id="table_id" class="display school-table" cellspacing="0" width="100%">
          <thead>
            <tr>
              ...
              <th>@lang('common.is_mandatory')</th>
            ...
            @if (isset($studentInvoices))
              @foreach ($studentInvoices as $key => $studentInvoice)
                @php
                  ...
                  $feeType = ($studentInvoice->Tid) ? getFeeType($studentInvoice->Tid->fees_type) : '';
                ...
                <tr>
                  ...
                  <td>{{ $feeType->fessGroup->name ?? '' }} Fees</td>
                  <td>
                    @isset($feeType->is_mandatory)
                    @if ($feeType->is_mandatory == 1)
                      <button type="button" class="primary-btn bg-success text-white small">Yes</button>
                    @else
                      <button type="button" class="primary-btn bg-danger small text-white">No</button>
                    @endif
                    @endisset
                  </td>
                  <td>{{ $amount }}</td>
                      ...
                      <div class="dropdown-menu dropdown-menu-right">
                        @if (isset($role) && $role == 'admin')
                          @if (userPermission(1141))
                            <a class="dropdown-item viewPaymentDetail"
                              data-id="{{ $studentInvoice->id }}">@lang('inventory.view_payment')</a>
                          @endif
                          @if ($balance == 0)
                          ...
                          @else
                            @if ($paid_amount > 0)
                              ...
                              @if (userPermission(1144))
                                <a class="dropdown-item"
                                  href="{{ route('fees.add-fees-payment', $studentInvoice->id) }}">@lang('inventory.make_payment')</a>
                              @endif
                            @else
                              @if (userPermission(1142))
                                <a class="dropdown-item"
                              ...
                              @if (userPermission(1144))
                                <a class="dropdown-item"
                                  href="{{ route('fees.add-fees-payment', $studentInvoice->id) }}">@lang('inventory.add_payment')</a>
                              @endif

                              @if (userPermission(1145))
                                <a class="dropdown-item"
                                  href="{{ route('fees.fees-invoice-edit', $studentInvoice->id) }}">@lang('common.edit')</a>
                              @endif

                              @if (userPermission(1146))
                                <a class="dropdown-item" data-toggle="modal"
                                  data-target="#deleteFeesPayment{{ $studentInvoice->id }}"
