...
class SmOnlineExamController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $sections = SmSection::where('academic_id', getAcademicId())->get();
            $subjects = SmSubject::where('academic_id', getAcademicId())->get();
            $categories = SmStudentCategory::where('academic_id', getAcademicId())->get();
            $present_date_time = date("Y-m-d H:i:s");
            $present_time = date("H:i:s");

            return view('backEnd.examination.online_exam', compact('online_exams', 'classes', 'sections', 'subjects', 'present_date_time', 'present_time', 'categories'));
        } catch (\Exception $e) {
    ...
    public function store(SmOnlineExamRequest $request)
    {
        ...
        try {
            ...
            foreach ($request->section as $section) {
                ...
                $online_exam->category_type = ($request->category_type == "student") ? 1 : 0;
            ...
            // dd($online_exam->exam_date);

            // get all students in sections
            if ($request->category_type == "student") {
                $students = SmStudent::whereIn('section_id', $request->section)->where('student_category_id', $request->category)
                    ->get();
            } else {
                $students = SmStudent::where('status', 0)
            ...
            foreach ($students as $key => $student) {
                ...
                // [student_name], [exam_type], [exam_date], [exam_time]
                $data['student_name'] = $student->full_name;
                $data['subject'] = ($request->category_type == "student") ? "Online Exam " : "Entrance Admission Exam ";
                $data['exam_type'] = ($request->category_type == "student") ? 'Online Exam For ' . $online_exam->class->name : 'Entrance Examination for ' . $online_exam->class->name;
                $data['exam_date'] = $online_exam->date;
                $data['exam_time'] = $online_exam->start_time;
                // send email to student
                @send_mail($student->email, $student->full_name, 'exam_schedule_for_student', $data);
                // send email to parent
                // [parent_name], [student_name], [exam_type], [exam_date], [exam_time]
                $parentData['student_name'] = $student->name;
                $parentData['parent_name'] = $student->parents->fathers_name;
                $parentData['exam_type'] = 'Entrance Examination for your children ' . $student->full_name;
                $data['subject'] = "Entrance Admission Exam for your child";
                $parentData['exam_date'] = $online_exam->date;
                $parentData['exam_time'] = $online_exam->start_time;
                @send_mail($student->email, $student->full_name, 'exam_schedule_for_parent', $parentData);
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            return view('backEnd.examination.online_exam', compact('online_exams', 'classes', 'sections', 'subjects', 'online_exam', 'present_date_time', 'present_time', 'categories'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function onlineExamPublish($id)
    {
        ...
        try {
            $time_zone_setup = SmGeneralSettings::join('sm_time_zones', 'sm_time_zones.id', '=', 'sm_general_settings.time_zone_id')
                ->where('school_id', Auth::user()->school_id)->first();
            date_default_timezone_set($time_zone_setup->time_zone);
            $present_date_time = date("Y-m-d H:i:s a");

            $publish = SmOnlineExam::find($id);

            // dd($present_date_time);
    ...
    public function onlineExamPublishClose($id)
    {
        ...
        try {
            ...
            $publish->status = 2;
            $publish->save();
            Toastr::success('Exam Closed Successfully', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function onlineExamMarksRegister($id)
    {
        ...
        try {
            ...
            $students = SmStudent::with('class', 'section')->where('class_id', $online_exam_question->class_id)
                ...
                ->where('status', $online_exam_question->category_type)
                ->get();
    ...
    public function onlineExamResult($id)
    {
        try {
            $online_exam_question = SmOnlineExam::find($id);
            // dd($online_exam_question);
            $students = SmStudent::where('class_id', $online_exam_question->class_id)
                ->where('section_id', $online_exam_question->section_id)
    ...
    public function onlineExamMarking($exam_id, $s_id)
    {
        ...
        try {
            ...
            if ($online_exam_info->auto_mark == 1) {
                ...
                if (!$take_online_exam) {
                    Toastr::info('Student has not attempted the exam', 'Failed');
                    return redirect()->back();
                }
                ...
                $take_online_exam = SmStudentTakeOnlineExam::where('online_exam_id', $exam_id)->where('student_id', $s_id)->where('academic_id', getAcademicId())->first();
                if (!$take_online_exam) {
                    Toastr::info('Student has not attempted the exam', 'Info');
                    return redirect()->back();
                }
