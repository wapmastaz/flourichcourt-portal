...
class SmStudentAdmissionController extends Controller
{
    ...
    public function studentAdmissionLists(Request $request)
    {
        // dd('hello');
        try {
            $classes = SmClass::where('active_status', 1)
            ...
            return view('backEnd.studentInformation.student_admissions_list', compact('classes', 'sessions'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function grantStudentAdmission(Request $request)
    {
        // dd($request->all());
        // $student_detail = SmStudent::find($request->id);
        ...
        try {
            ...
            $student = SmStudent::find($request->id);
            $section = '';
            // $student->student_category_id = 2;
            if ($student->section_id) {
                if ($student->section->section_name == "admission" || $student->section->section_name == "Admission") {

                    $sections = SmSection::where('academic_id', getAcademicId())
                        ->where('section_name', '!=', 'admission')
                        ->get();

                    foreach ($sections as $key => $value) {
                        if ($value->students->count() < $value->capacity) {
                            $section = $value;
                            break;
                        }
                    }
            ...
            $student->user->active_status = 1;
            $student->user->save();

            // send message notification to student and parent

            // send notification to child
            $notification = new SmNotification();
            $notification->user_id = $student->user->id;
            $notification->role_id = 2; //student
            $notification->date = date('Y-m-d');
            $notification->message = 'Admission Granted Successfully';
            $notification->url = 'student-dashboard';
            ...
            $notification = new SmNotification();
            $notification->user_id = $student->parents->user_id;
            $notification->role_id = 3; //parent
            $notification->date = date('Y-m-d');
            $notification->message = 'Admission Granted Successfully';
            $notification->url = 'parent-dashboard';
            $notification->school_id = 1;
            $notification->academic_id = getAcademicId();
            $notification->save();

            // send mail
            $compact['slug'] = 'admission_approved';
            $compact['id'] = $student->id;

            @send_mail($student->email, $student->full_name, "student_admission_approved", $compact);

            @send_mail($student->email, $student->full_name, "parent_admission_approved", $compact);

            DB::commit();

            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse(null, 'Student has been granted admission successfully');
            }
            Toastr::success('Operation successful', 'Success');
            return redirect()->back();
        } catch (\Exception $e) {
    ...
    public function bulkGrantStudentAdmission(Request $request)
    {
        try {
            $online_exam_question = SmOnlineExam::find($request->online_exam_id);
            ...
            if (count($present_students) == 0) {
                Toastr::info('No Students present for the exams', 'Info');
                return redirect()->back();
            }
            foreach ($present_students as $key => $student) {
                $obtained_marks = SmOnlineExam::obtainedMarks($online_exam_question->id, $student);

                $result = ($obtained_marks->total_marks * 100) / $total_marks;
                if ($result >= $online_exam_question->percentage) {
                    $currentStudent = SmStudent::find($student);
                    ...
                    $notification = new SmNotification();
                    $notification->user_id = $currentStudent->user->id;
                    $notification->role_id = 2; //student
                    $notification->date = date('Y-m-d');
                    $notification->message = 'Admission Granted Successfully';
                    $notification->url = 'student-dashboard';
                    ...
                    $notification = new SmNotification();
                    $notification->user_id = $currentStudent->parents->user_id;
                    $notification->role_id = 3; //parent
                    $notification->date = date('Y-m-d');
                    $notification->message = 'Admission Granted Successfully';
                    $notification->url = 'parent-dashboard';
