...
class DatatableQueryController extends Controller
{
    public function studentDetailsDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            // $students->where('sm_students.student_category_id', 2);
            ...
            $students = $students->where('sm_students.school_id', Auth::user()->school_id)
                ...
                ->with(array('parents' => function ($query) {
                    $query->select('id', 'fathers_name', 'mothers_name', 'guardians_name');
                }))
    ...
    public function studentAdmissionDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            // return "Ajax";
            $students = SmStudent::with(['class', 'section', 'category', 'gender'])->select('sm_students.*');
            $students->where('sm_students.status', 0);
            // dd($students->get());
            $students->where('sm_students.active_status', 1);
            if ($request->class != "") {
                $students->where('sm_students.class_id', $request->class);
            ...
            return Datatables::of($students)
                ...
                ->addColumn('assign_exam', function ($row) {
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" href="' . route('student_view', [$row->id]) . '">' . app('translator')->get('common.view') . '</a>' .
                        (userPermission(66) === true ? '<a class="dropdown-item" href="' . route('student_edit', [$row->id]) . '">' . app('translator')->get('common.edit') . '</a>' : '') .

                        (userPermission(67) === true ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="dropdown-item" href="#"  >' . app('translator')->get('common.disable') . '</a></span>' :
                        '<a onclick="deleteId(' . $row->id . ');" class="dropdown-item" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="' . $row->id . '"  >' . app('translator')->get('common.disable') . '</a>') : '') .

                        (userPermission(67) === true ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="dropdown-item" href="#"  >' . app('translator')->get('common.grant_admission') . '</a></span>' :
                        '<a onclick="grantAdmissionId(' . $row->id . ');" class="dropdown-item" href="#" data-toggle="modal" data-target="#grantStudentAdmissionModal" data-id="' . $row->id . '"  >' . app('translator')->get('common.grant_admission') . '</a>') : '') .
