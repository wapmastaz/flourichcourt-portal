...
class SmStudentAdmissionController extends Controller
{
    ...
    public function admissionPic(Request $r)
    {
        try {
    ...
    public function studentUpdatePic(Request $r, $id)
    {
        $validator = Validator::make($r->all(), [
            'logo_pic' => 'sometimes|required|mimes:jpg,png|max:40000',
    ...
    public function importStudent()
    {
        try {
            ...
            $genders = SmBaseSetup::where('base_group_id', 1)
                ->where('school_id', Auth::user()->school_id)
                ->get();
            $blood_groups = SmBaseSetup::where('base_group_id', 3)
                ->where('school_id', Auth::user()->school_id)
                ->get();
            $religions = SmBaseSetup::where('base_group_id', 2)
                ->where('school_id', Auth::user()->school_id)
                ->get();
            $sessions = SmAcademicYear::where('school_id', Auth::user()->school_id)->get();
    ...
    {
        try {
            $studentsArray = ['admission_number', 'roll_no', 'first_name', 'last_name', 'date_of_birth', 'religion', 'gender', 'caste', 'mobile', 'email', 'admission_date', 'blood_group', 'height', 'weight', 'father_name','father_email', 'father_phone', 'father_occupation', 'mother_name','mother_email', 'mother_phone', 'mother_occupation', 'guardian_name', 'relation', 'guardian_email', 'guardian_phone', 'guardian_occupation', 'guardian_address', 'current_address', 'permanent_address', 'bank_account_no', 'bank_name', 'national_identification_no', 'local_identification_no', 'previous_school_details', 'note'];
                ...
                if (!empty($data)) {
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if ($value->father_email != "") {
                            $chk = DB::table('sm_parents')->where('fathers_email_address', $value->father_email)->where('school_id', Auth::user()->school_id)->count();
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error('Parent Email address should be unique.', 'Failed');
                                return redirect()->back();
                            }
                        }

                        try {
                            $studentCategory = SmStudentCategory::where('category_name', 'normal')
                                ->orWhere('category_name', 'Normal')
                            ...
                            $admissionNumber = '';
                            if ($value->admission_number == null || $value->admission_number == '') {
                                $admissionNumber = getAdmissionNumber();
                            } else {
                                $ad_check = SmStudent::where('admission_no', (int) $value->admission_number)->where('school_id', Auth::user()->school_id)->first();
                                
                                $admissionNumber = ($ad_check) ? getAdmissionNumber() : $value->admission_number;
                            }
                            $academic_year = SmAcademicYear::find($request->session);
                            ...
                            if (empty($value->email)) {
                                $user_stu->username = $admissionNumber;
                            } else {
                            ...
                            $user_stu->password = Hash::make($studentPassword);
                            ...
                            try {
                                ...
                                if (empty($value->father_email)) {
                                    $data_parent['email'] = 'par_' . $admissionNumber;

                                    $user_parent->username = 'par_' . $admissionNumber;
                                } else {

                                    $data_parent['email'] = $value->father_email;

                                    $user_parent->username = 'par_' . $admissionNumber;
                                }

                                switch(strtolower($value->relation)){
                                    case 'father':
                                        $user_parent->email = $value->father_email;
                                        break;
                                    case 'mother':
                                        $user_parent->email = $value->mother_email;
                                        break;
                                    case 'guardian':
                                ...
                                $user_parent->password = Hash::make($parentPassword);
                                $user_parent->school_id = Auth::user()->school_id;
                                ...
                                try {
                                    ...
                                    if (
                                        ...
                                        $value->relation == 'father' ||
                                        $value->relation == 'F' ||
                                        strtolower($value->relation) == 'father'
                                    ) {
                                        $relationFull = 'Father';
                                        $relation = 'father';
                                    } elseif (
                                        ...
                                        $value->relation == 'mother' ||
                                        strtolower($value->relation) == 'mother'
                                    ) {
                                        $relationFull = 'Mother';
                                        $relation = 'mother';
                                    } elseif (
                                        ...
                                        $value->relation == 'guardian' ||
                                        strtolower($value->relation) == 'guardian'
                                    ) {
                                        $relationFull = 'Guardian';
                                        $relation = 'guardian';
                                    }
                                    else{
                                        $relationFull = 'Parent';
                                    ...
                                    $parent->fathers_email_address = $value->father_email;
                                    $parent->fathers_occupation = $value->father_occupation;
                                    $parent->mothers_name = $value->mother_name;
                                    $parent->mothers_mobile = $value->mother_phone;
                                    $parent->mothers_email_address = $value->mother_email;
                                    ...
                                    try {
                                        ...
                                        $student->session_id = $request->session;
                                        $student->user_id = $user_stu->id;
                                        $student->student_category_id = ($studentCategory) ? $studentCategory->id : null;

                                        $student->parent_id = $parent->id;
                                        $student->role_id = 2;
                                        $student->status = 1;
                                        $student->admission_no = $admissionNumber;
                                        ...
                                        if ($value->father_email != "") {
                                            $user_info[] = array('email' => $value->father_email, 'username' => $data_parent['email']);
                                        }
                                    } catch (\Illuminate\Database\QueryException $e) {
                        ...
                        if ($parent) {
                            ...
                            $compact['relation'] = $parent->guardians_relation;
                            if ($parent->relation == "parent") {
                                // $guardianName = $parent->fathers_name;
                                $compact['user_email'] = $parent->fathers_email_address;
                                @send_mail($parent->fathers_email_address, $parent->fathers_name, "parent_login_credentials", $compact);
                                
                                @send_mail($parent->mothers_email_address, $parent->mothers_name, "parent_login_credentials", $compact);
                            }elseif ($parent->relation == "father") {
                                $compact['user_email'] = $parent->fathers_email_address;
                                @send_mail($parent->fathers_email_address, $parent->fathers_name, "parent_login_credentials", $compact);
                            }
                            elseif ($parent->relation == "mother") {
                                $compact['user_email'] = $parent->mothers_email_address;
                                @send_mail($parent->mothers_email_address, $parent->mothers_name, "parent_login_credentials", $compact);
                            } elseif ($parent->relation == "guardian") {
                                $compact['user_email'] = $parent->guardians_email;
                                @send_mail($parent->guardians_email, $parent->gaurdians_name, "parent_login_credentials", $compact);
                            }
                        ...
                        if ($student) {
                            ...
                            @send_mail($student->email, $student->full_name, "student_login_credentials", $compact);
                            //@send_sms($request->phone_number, 'student_admission', $compact);
