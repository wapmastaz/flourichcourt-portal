              ...
              @foreach ($sm_weekends as $sm_weekend)
                <li class="nav-item">
                  <a class="nav-link  {{ Session::get('session_day_id') != null ? (Session::get('session_day_id') == $sm_weekend->id ? 'active' : '') : ($loop->index == 0 ? 'active' : '') }} tab_link "
                    href="{{ $sm_weekend->name }}" data-sm_weekend_id="{{ $sm_weekend->id }}" role="tab"
                    data-toggle="tab">{{ @$sm_weekend->name }}</a>
                </li>
              @endforeach

              <li class="nav-item edit-button">
                @if (userPermission(66))
                  <button class="primary-btn small fix-gr-bg" onclick="addRowInRoutine();" id="addRowBtn"> <span
