...
<body style="font-family: 'dejavu sans', sans-serif;">
  ...
  <div class="container-fluid" id="pdf">
    ...
    <table cellspacing="0" width="100%">
      <tr>
        <td>
          <img src="{{ url('/') }}/{{ @generalSetting()->logo }}" style="padding-top: 20px; max-width: 250px"
            alt="">
        </td>
    ...
    <table class="table table-bordered table-striped" style="width: 100%; table-layout: fixed">
      ...
      @for ($i = 0; $i < $height; $i++)
            ...
            @foreach ($sm_weekends as $sm_weekend)
              <td style="padding-top:0px ;padding-bottom:0px;">
                ...
                @if ($classes && gv($classes, $i))
                  <span style="font-size:10px !important;"> {{ date('h:i A', strtotime(@$classes[$i]['start_time'])) }}
        ...
        <tr>
          <td>@lang('common.details')
            {{-- {{ __('Subject') }} <br> {{ __('Teacher') }} --}}
          </td>
          @foreach ($tr as $days)
            @foreach ($sm_weekends as $sm_weekend)
              <td style="padding-top:0px ;padding-bottom:0px;">
