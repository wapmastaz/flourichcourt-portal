...
<body class="admin" style=" @if(@activeStyle()->id==1) {{$css}} @else background:{{@activeStyle()->dashboardbackground}} !important; @endif ">
    ...
    @php
        ...
        $tt = curl_get_file_contents(url('/').'/'.$generalSetting->logo, false, stream_context_create($arrContextOptions));
        } else {
        $tt = curl_get_file_contents(url('/').'/public/uploads/settings/logo.png', false, stream_context_create($arrContextOptions));
        }
    @endphp
