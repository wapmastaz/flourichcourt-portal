          ...
          <div class="collapse navbar-collapse flex-end" id="navbarSupportedContent">
            <ul class="nav navbar-nav mr-auto nav-buttons flex-sm-row">
              ...
              @if (generalSetting()->dashboard_btn == 1)
                @if (Auth::user()->role_id == $coltroller_role)
                  <li class="nav-item">
                    <a class="primary-btn white mr-10" href="{{ route('admin-dashboard') }}">@lang('common.dashboard')</a>
                  </li>
                @endif
              @endif
              ...
              <li class="nav-item active">
                ...
                <select class="niceSelect infix_session" id="infix_session">
                  ...
                  @foreach (academicYears() as $academic_year)
                    <option value="{{ @$academic_year->id }}" {{ getAcademicId() == @$academic_year->id ? 'selected' : '' }}>
                  ...
                  <li class="nav-item active">
                    <select class="niceSelect infix_theme_style" id="infix_theme_style">
                      ...
                      @foreach ($styles as $style)
                        <option value="{{ $style->id }}" {{ activeStyle()->id == $style->id ? 'selected' : '' }}>
                          {{ $style->style_name }}</option>
                      @endforeach
                    </select>
                  </li>
                </ul>
            ...
            <ul class="nav navbar-nav right-navbar">
                  ...
                  <select class="niceSelect languageChange" name="languageChange" id="languageChange">
                    ...
                    @foreach ($languages as $lang)
                      <option data-display="{{ $lang->native }}" value="{{ $lang->language_universal }}"
                        {{ $lang->language_universal == userLanguage() ? 'selected' : '' }}>{{ $lang->native }}</option>
                    @endforeach
                  </select>
                </li>
              @endif
                    ...
                    <div class="white-box">
                      ...
                      <div class="sctolled_notify">
                        ...
                        @foreach ($notifications as $notification)
                          <a class="dropdown-item pos-re" href="{{ route('notification-show', $notification->id) }}">
                            ...
                            <div class="profile_single_notification d-flex justify-content-between align-items-center">
                              ...
                              <div class="text-right bell_time">
                                <p class="time text-uppercase">
                                  {{ date('h.i a', strtotime($notification->created_at)) }}</p>
                                <p class="date">
              ...
              <li>
                ...
                <form id="logout-form"
                  action="{{ Auth::user()->role_id == 2 ? route('student-logout') : route('logout') }}" method="POST"
                  style="display: none;">
