                ...
                <div class="col-lg-12" id="myFrame">
                    <div class="table-responsive">
                        <table id="" class="display school-table school-table-style-parent-fees" cellspacing="0"
                            ...
                            <thead>
                            <tr>
                                <th class="nowrap">@lang('fees.fees_groups') </th>
                                        ...
                                        @if($rest_amount =! 0)
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right">
                                                    ...
                                                    <!-- Start KhaltiPayment Checking  -->

                                                        @if ( (moduleStatusCheck('KhaltiPayment') == TRUE)  && ($balance_amount > 0) )
                                                            @php
                                                                $is_khalti = DB::table('sm_payment_gateway_settings')->where('gateway_name','Khalti')->where('school_id', Auth::user()->school_id)->first('gateway_publisher_key');

                                                            @endphp

                                                            <div class="pay">
                                                                <button class="dropdown-item btn filled small khalti-payment-button" data-amount="{{$balance_amount}}"
                                                                        data-assignid = "{{$fees_assigned->id}}" data-feestypeid = "{{$fees_assigned->feesGroupMaster->fees_type_id}}">
                                                    ...
                                                    @if($already_add=="" && $balance_amount !=0)
                                                        ...
                                                        @if(@$data['bank_info']->active_status == 1 || @$data['cheque_info']->active_status == 1 )
                                                            ...
                                                            <a class="dropdown-item modalLink"
                                                               ...
                                                               href="{{route('fees-generate-modal-child', [@$balance_amount, $fees_assigned->student_id, $fees_assigned->feesGroupMaster->fees_type_id,$fees_assigned->id])}}"> @lang('fees.add_bank_payment')</a>
                                                        ...
                                                        @if($balance_amount !=0)
                                                            ...
                                                            @if ($already_add!="")
                                                                <a class="dropdown-item modalLink"
                                                                   data-modal-size="modal-lg"
                                                                   title="{{$fees_assigned->feesGroupMaster->feesGroups->name.': '. $fees_assigned->feesGroupMaster->feesTypes->name}}"
                                                                   href="{{route('fees-generate-modal-child-view', [$fees_assigned->student_id,$fees_assigned->feesGroupMaster->fees_type_id,$fees_assigned->id])}}">@lang('fees.view_bank_payment')</a>
                                                                ...
                                                                @if(@$already_add->approve_status == 0)
                                                                    <a onclick="deleteId({{@$already_add->id}});"
                                                                       ...
                                                                       data-id="{{@$already_add->id}}">@lang('fees.delete_bank_payment')</a>
                                                    ...
                                                    @endif
                                                    @php
                                                        $is_paypal = DB::table('sm_payment_methhods')->where('method','PayPal')->where('school_id', Auth::user()->school_id)->where('active_status',1)->first()
                                                    @endphp
                                                    ...
                                                    @php
                                                        $is_paystack = DB::table('sm_payment_methhods')->where('method','Paystack')->where('school_id', Auth::user()->school_id)->where('active_status',1)->first()
                                                    @endphp
                                                    ...
                                                    @php
                                                        $is_active = DB::table('sm_payment_methhods')->where('method','XenditPayment')->where('active_status',1)->where('school_id', Auth::user()->school_id)->first()
                                                    @endphp
                                                    ...
                                                    @php
                                                        $is_active = DB::table('sm_payment_methhods')->where('method','RazorPay')->where('active_status',1)->where('school_id', Auth::user()->school_id)->first()
                                                    @endphp
                                                ...
                                                <!-- Start Xendit Checking  -->
                                                    ...
                                                    <!-- Start Raudhahpay Checking  -->
                                                        @php
                                                            $is_active = DB::table('sm_payment_methhods')->where('method','Raudhahpay')->where('active_status',1)->where('school_id', Auth::user()->school_id)->first()
                                                        @endphp
    ...
    @push('script')
        ...
        <script>
            $(document).on('click', '.khalti-payment-button', function(){
                ...
                var productinfo = feesTypeId + '_' + assignId ;
